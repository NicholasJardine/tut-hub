      ...
      try {
        if (tutorId) {
          const [availabilityResponse, bookingsResponse] = await Promise.all([
            fetch(`/api/availability?tutorId=${tutorId}`),
              ...
              <React.Fragment key={timeIndex}>
                ...
                {weekDays.map((dayObj, dayIndex) => {
                  const matchingBooking = bookings.find(
                    (booking) => new Date(booking.booking_date).toISOString().split('T')[0] === formatDateToYMD(dayObj.fullDate) &&
...
>
  {matchingBooking ? (
    ...
    <div>
      <button className="btn btn-primary text-xs opacity-1">Book Slot</button>
    </div>
  ) : (
